  ...
  // MD5 (implémentation pure, sans dépendance) — utilisé pour les URL Gravatar
  function md5(s) {
    ...
    function tb(str) {
      ...
      var n = str.length, b = [], i;
      for (i = 0; i < n; i++) b[i >> 2] |= (str.charCodeAt(i) & 0xff) << ((i % 4) * 8);
      b[n >> 2] |= 0x80 << ((n % 4) * 8);
      b[(((n + 8) >> 6) + 1) * 16 - 2] = n * 8;
      return b;
    }
    function hex(num) { var s = '', j; for (j = 0; j < 4; j++) s += ('0' + ((num >> (j * 8)) & 0xff).toString(16)).slice(-2); return s; }
    var x = tb(s), a = 1732584193, b = -271733879, c = -1732584194, d = 271733878, i;
